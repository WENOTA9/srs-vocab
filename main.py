...
def main(page: ft.Page):
    ...
    # ── TTS: Android 偵測 (platform.system() 在 Android 上回傳 "Linux") ──
    _on_android = os.path.exists("/system/build.prop") or os.environ.get("ANDROID_ROOT") is not None
    ...
    def load_mcq():
        ...
        if reps == 0:
            ...
            fb_extra.opacity = 0
            speak_btn.visible = not _on_android; speak_btn.icon_color = C()["primary"]
    ...
    # Flet 0.25+ 在封包 EXE 有視窗無法關閉的 bug (Flet issue #4808, #5180)
    # 解法：偵測到斷線 → 背景清理 + os._exit(0) 強制結束程序
    def _force_exit(e):
        try: S.cleanup()
        except: pass
        import os
        os._exit(0)
    page.on_disconnect = _force_exit
